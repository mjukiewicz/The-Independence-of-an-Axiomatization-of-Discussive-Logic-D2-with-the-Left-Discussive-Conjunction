...
['~(p v q) > (~p ^ ~q)', False],
['~(~(~p) v q) > ~(p v q)', False],
['(~(p v q) > r) > ((~p > q) v r)', False],
['~((p v q) v r) > ~(p v (q v r))', False],
['~((p > q) v r) > (p ^ ~(q v r))', False],
['~((p ^ q) v r) > (q > ~(p v r))', False],
['~(~(p v q) v r) > (~(~p v r) v (~(~q v r)))', False],
['~(~(p > q) v r) > (p > ~(~q v r))', False],
['~(~(p ^ q) v r) > (~(~p v r) ^ q)', False],
]

def matrix_generator(first_element, second_element=0, third_element=0, fourth_element=0):
    # numbers_list=[0]*6
    with open("v3_step6.txt") as file1:
        for line1 in file1:

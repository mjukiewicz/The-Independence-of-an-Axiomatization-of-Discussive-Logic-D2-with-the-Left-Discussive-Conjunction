...
def main(first_element, dataset):
    ...
    for index, values_to_check in enumerate(matrix_generator(first_element)):
        ...
        for indx, element in enumerate(dataset):
            r=print_table(element[0], element[1], element[2], False, element[3], list_of_dic, n_deginated_val=1)
            if not r: n_inccorect+=1
            if n_inccorect>=1: break
        if values_to_check[9]==0 and values_to_check[11]==0 and values_to_check[15]==0 and values_to_check[17]==2 and n_inccorect==0:
